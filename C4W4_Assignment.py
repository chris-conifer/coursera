...
def parse_data_from_file(filename):
    ...
    with open(filename) as csvfile:
        ...
        for row in reader:
            # Access the temperature from the row (index 1) and convert it to a float
            temperature = float(row[1])
            temperatures.append(temperature)

            # Append the current time step to the times list
            times.append(time_step)

            # Increment the time step counter for the next iteration
            time_step += 1
